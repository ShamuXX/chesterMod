...
public class MessageArgument implements SignedArgument<MessageArgument.Message> {
   ...
   public static MessageArgument.ChatMessage getChatMessage(CommandContext<CommandSourceStack> p_232164_, String p_232165_) throws CommandSyntaxException {
      ...
      MessageSignature messagesignature = commandsigningcontext.getArgumentSignature(p_232165_);
      boolean flag = commandsigningcontext.signedArgumentPreview(p_232165_);
      ChatSender chatsender = p_232164_.getSource().asChatSender();
      return messagesignature.isValid(chatsender.uuid()) ? new MessageArgument.ChatMessage(messageargument$message.text, component, messagesignature, flag) : new MessageArgument.ChatMessage(messageargument$message.text, component, MessageSignature.unsigned(), false);
   }
   ...
   public Component getPlainSignableComponent(MessageArgument.Message p_232159_) {
   ...
   public static record ChatMessage(String plain, Component formatted, MessageSignature signature, boolean signedPreview) {
      public CompletableFuture<FilteredText<PlayerChatMessage>> resolve(CommandSourceStack p_232175_) {
         CompletableFuture<FilteredText<PlayerChatMessage>> completablefuture = this.filterComponent(p_232175_, this.formatted).thenComposeAsync((p_232189_) -> {
            ChatDecorator chatdecorator = p_232175_.getServer().getChatDecorator();
            return chatdecorator.decorateChat(p_232175_.getPlayer(), p_232189_, this.signature, this.signedPreview);
         }, p_232175_.getServer()).thenApply((p_238193_) -> {
            PlayerChatMessage playerchatmessage = this.getSignedMessage(p_238193_);
            if (playerchatmessage != null) {
               this.verify(p_232175_, playerchatmessage);
            }
      ...
      @Nullable
      private PlayerChatMessage getSignedMessage(FilteredText<PlayerChatMessage> p_238377_) {
         if (this.signature.isValid()) {
            return this.signedPreview ? p_238377_.raw() : PlayerChatMessage.signed(this.plain, this.signature);
         } else {
            return null;
         }
      }

      private void verify(CommandSourceStack p_238405_, PlayerChatMessage p_238406_) {
         if (!p_238406_.verify(p_238405_)) {
            MessageArgument.LOGGER.warn("{} sent message with invalid signature: '{}'", p_238405_.getDisplayName().getString(), p_238406_.signedContent().getString());
         }

      }

      private CompletableFuture<FilteredText<Component>> filterComponent(CommandSourceStack p_232180_, Component p_232181_) {
         ServerPlayer serverplayer = p_232180_.getPlayer();
         return serverplayer != null ? serverplayer.getTextFilter().processStreamComponent(p_232181_) : CompletableFuture.completedFuture(FilteredText.passThrough(p_232181_));
      }
   }
