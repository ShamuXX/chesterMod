 ...
 * }</pre>
 * <p>
 * Due to size of the ping packet (32767 UTF-16 code points of JSON data) this
 * could exceed this limit and
 * cause issues. To work around this, a truncation mechanism was introduced, to
 * heuristically truncate the size of the
 * data, at the expense of making the compatibility info on the server screen
 * inaccurate.
 *
 * <p>
 * Modern versions will send binary data, which is encoded in a custom format
 * optimized for UTF-16 code point count.
 * See {@link Serializer#encodeOptimized(ByteBuf)} and
 * {@link Serializer#decodeOptimized(String)}.
 * Essentially 15 bits of binary data are encoded into every UTF-16 code point.
 * The resulting string is then stored in
 * the "d" property of the resulting JSON.
 *
 * <p>
 * For the format of the binary data see
 * {@link Serializer#serialize(ServerStatusPing)}.
 *
 * <p>
 * The "channels" and "mods" properties are retained for backwards
 * compatibility,
 * but left empty. A client that cannot read the old format would not be able to
 * connect anyways, but the properties
...
public class ServerStatusPing {
    ...
    public ServerStatusPing() {
        ...
        ModList.get()
                .forEachModContainer((modid, mc) -> mods.put(modid,
                        mc.getCustomExtension(IExtensionPoint.DisplayTest.class)
    ...
    private ServerStatusPing(Map<ResourceLocation, Pair<String, Boolean>> deserialized, Map<String, String> modMarkers,
            int fmlNetVer, boolean truncated) {
    ...
    public boolean equals(Object o) {
        if (this == o)
            return true;
        if (!(o instanceof ServerStatusPing that))
            return false;
    ...
    public static class Serializer {
        public static ServerStatusPing deserialize(JsonObject forgeData) {
            try {
                ...
                final Map<ResourceLocation, Pair<String, Boolean>> channels = StreamSupport
                        .stream(GsonHelper.getAsJsonArray(forgeData, "channels").spliterator(), false)
                        .map(JsonElement::getAsJsonObject)
                        .collect(Collectors.toMap(jo -> new ResourceLocation(GsonHelper.getAsString(jo, "res")),
                                jo -> Pair.of(GsonHelper.getAsString(jo, "version"),
                                        GsonHelper.getAsBoolean(jo, "required"))));

                final Map<String, String> mods = StreamSupport
                        .stream(GsonHelper.getAsJsonArray(forgeData, "mods").spliterator(), false)
                        .map(JsonElement::getAsJsonObject)
                        .collect(Collectors.toMap(jo -> GsonHelper.getAsString(jo, "modId"),
        ...
        public static JsonObject serialize(ServerStatusPing forgeData) {
            ...
            // 1. Try and group channels by ModID, this relies on the assumption that a mod
            // "chestermod" uses a channel
            // like "chestermod:network". In that case only the "path" of the
            // ResourceLocation is written
            // 2. Avoid sending IGNORESERVERONLY in plain text, instead use a flag (if set,
            // no version string is sent)
            //
            // The size can be estimated as follows (assuming there are no non-mod network
            // channels)
            // bytes = 2
            // + mod_count * (avg_mod_id_length + avg_mod_version_length + 1)
            // + (mod_count * avg_channel_count_per_mod) * (avg_mod_channel_length +
            // avg_mod_channel_version_length + 1)
            // + 1
            // for 600 mods with an average ModID and channel length of 20, an average
            // channel of 1 per mod and an
            // average version length of 5 this turns out to be 31203 bytes, which easily
            // fits into the upper limit of
            // roughly 60000 bytes. As such it is estimated that the upper limit will never
            // be reached.
            ...
            // legacy versions see truncated lists, modern versions ignore this truncated
            // flag (binary data has its own)
            ...
            try {
                ...
                for (var i = 0; i < modsSize; i++) {
                    ...
                    for (var i1 = 0; i1 < channelSize; i1++) {
                        ...
                        channels.put(new ResourceLocation(modId, channelName),
                                Pair.of(channelVersion, requiredOnClient));
                    }
        ...
        /**
         * Encode given ByteBuf to a String. This is optimized for UTF-16 Code-Point
         * count.
